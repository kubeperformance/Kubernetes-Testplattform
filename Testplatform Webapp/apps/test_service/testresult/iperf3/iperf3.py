import json
from apps.test_service.models import iPerfTestResult
        ...
        if 'error' in self.json:
            ...
            if self.protocol == 'tcp':
                ...
                for i in range(0, 10):
                    ...
                    # Bytes to megabytes
                    self.megabytes_per_second.append(self.json['intervals'][i]['streams'][0]['bytes'] / 1000000) 
...
class iPerf3ResultService:
    '''
    A class capsuling the process of transforming a iperf3 json output into a result object.
    
    '''
    
    
    @staticmethod
    def create_result_object(json_output):
        '''
        Transforms the CLI-Output of a iperf3 test in iPerfTestResult object. The function
        uses the TestResult class for mapping the output.

            Parameters:
                        json_output (str): result as json string
            Returns:
                        iPerf_result_object (iPerfTestResult): Result object containing the result information
        '''
        
        if json_output != '':

            json_output = json_output.lower().replace("'", '"')
            result = TestResult(json_output)        
            
            iPerf_result_object = iPerfTestResult()        

            iPerf_result_object.result_as_json = result.json
            iPerf_result_object.client_ip = result.local_host
            iPerf_result_object.server_ip = result.remote_host
            iPerf_result_object.protocol = result.protocol
            iPerf_result_object.duration = str(result.duration)
            iPerf_result_object.system_info = result.system_info

            if result.protocol == 'tcp':
                iPerf_result_object.sent_MB_s = result.sent_MB_s
                iPerf_result_object.received_MB_s = result.received_MB_s
                iPerf_result_object.transmission_rate_per_second = json.dumps(result.megabytes_per_second) 
        
        else:
            print("Created Empty ipfer3-result Object")
            iPerf_result_object = iPerfTestResult()  

        return iPerf_result_object
